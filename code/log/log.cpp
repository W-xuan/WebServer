...
namespace MicroWS
{
    ...
    void Log::init(int level = 1, const char *path, const char *suffix, int maxQueueSize)
    {
        ...
        snprintf(fileName, LOG_NAME_LEN - 1, "%s/%04d%02d_%02d%s", path_, t.tm_year + 1990, t.tm_mon + 1, t.tm_mday, suffix_);
        toDay_ = t.tm_mday;
    ...
    void Log::write(int level, const char *format, ...)
    {
        ...
        if (toDay_ != t.tm_mday || (lineCount_ && (lineCount_ % MAX_LINES == 0)))
        {
            ...
            {
                snprintf(newFile, LOG_NAME_LEN - 72, "%s/%s%s", path_, tail, (lineCount_ / MAX_LINES), suffix_);
            }
